...
int main()
{
	int n, mid1, mid2,mid3, sum1 = 0, sum2 = 0, sum = 0;
	printf("输入士兵的个数：");
	scanf_s("%d", &n);
	printf("输入士兵的横纵坐标:\n");
	for (int i = 0; i < n; i++)
		scanf_s("%d %d", &b[i], &a[i]);//输入士兵的坐标
	...
	*/
	...
	mid2 = SearchMid2(0, n - 1, n / 2 + 1);//mid2是排完后中位数
	...
	mid3 = SearchMid2(0, n - 1, n / 2 + 1);//mid2是排完后中位数
	...
	for (int i = 0; i < n; i++)
		sum1 += abs(a[i] - mid1);//所有士兵位置的纵坐标减去中位数的绝对值即为最小长度总和
	for (int i = 0; i < n; i++)
		sum2 += abs(b[i] - mid3);//所有士兵位置的横坐标减去中位数的绝对值即为最小长度总和

	sum = sum1 + sum2;

	printf("y轴方向上士兵移动的步数是%d\n", sum1);
	printf("x轴方向上士兵移动的步数是%d\n", sum2);
